...
module.exports = async function(table, match) {
  let objs = await mdb.postQuery(`
    select ${table}s.id, name, syns from ${table}s join (
      select id, group_concat(syn separator "::") as syns from (
        select id, name as syn from ${table}s union select ${table}s.id, ${table}s_syns.name from ${table}s join ${table}s_syns on ${table}s.id = ${table}s_syns.of
      ) t1 group by id
